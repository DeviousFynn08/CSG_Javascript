...
// inspiratie van hoofdstuk 2 opdracht 40
class Kikker {
    ...
    wordtGeraakt(auto) { // opdracht 40 uit hoofdstuk 2
...
class Auto { //hoofdstuk 2 opdracht 40
    constructor(png, celGrootte, raster) {
        this.afbeelding = png;
        this.breedte = celGrootte * 1.5;
        this.hoogte = celGrootte * 1.5;
        this.celGrootte = celGrootte;
        this.raster = raster;
        this.kolom = random(0, this.raster.aantalKolommen); // start links
        this.rij = floor(random(0, this.raster.aantalRijen));
        this.x = this.kolom * this.celGrootte;
        this.y = this.rij * this.celGrootte;
        ...
        this.snelheidAuto = this.baseSnelheid;
    }
    ...
    teken() {
        ...
        if (this.x > this.totaalbreed) {
            this.x = -this.breedte;
        }

        // waar vandaan? zet pls bij
...
class Demospel {
    ...
    nieuwLevel() {
        ...
        this.a = new Auto(pngAuto, this.raster.celGrootte, this.raster);
        ...
        this.a.snelheidAuto = max(1, floor(this.a.baseSnelheid * factor));
    ...
    update() {
        if (this.actief && !this.levelGehaald) {
            ...
            // verplaats en controleer elke auto
            for (var i = 0; i < this.autos.length; i++) {
                var auto = this.autos[i];
                auto.rijden();

                // botsingscontrole met rechthoek-overlap
                if (this.k.wordtGeraakt(auto)) {
                    if (!this.k.benGeraakt) {
                        this.k.benGeraakt = true;
                        this.gefaald = true;
                        this.actief = false;
                        if (pngBeat && pngBeat.isPlaying()) pngBeat.stop();
                        if (pngCrash) pngCrash.play();
                    }
                }
            }
            ...
            // level gehaald wanneer kikker de bovenkant van het scherm bereikt
            if (this.k.y <= -1 ) {
        ...
        // faal-animatie opvoeren als spel niet actief is en faalstatus true
        if (this.gefaald && !this.actief) {
            this.faalAnimatie = min(this.faalAnimatie + 3, 2550); // cap zodat s niet onbegrensd groeit
        }
    }

    geluid() { //ai prompt in word
    if (!pngBeat) return;

    const isPlaying = pngBeat.isPlaying();
    
    
    if (this.afgelopen && pngAllesgehaald) {
    ...
    if (this.tekenStartScherm && pngAllesgehaald && pngAllesgehaald.isPlaying()) {
        pngAllesgehaald.stop();
    }
    ...
    //tekst was aan het overlappen met elkaar en sommige laden niet in en kwam er niet uit, deze hier is ai gebruikt
...
Als je er klaar voor bent klik dan op spatie om te beginnen!`;
   ...
   tekenLevelScherm() {
    ...
    tekenSpelFaal() { //dit had ai gedaan
        
        if (this.k) {
            var s = 1 + this.faalAnimatie / 50; 
            image(this.k.afbeelding, canvas.width/2 - this.k.breedte*s/2, canvas.height/2 - this.k.hoogte*s/2, this.k.breedte*s, this.k.hoogte*s);
            image (pngBloed, canvas.width/2 - this.k.breedte*s/2, canvas.height/2 - this.k.hoogte*s/2, this.k.breedte*s, this.k.hoogte*s);
        }
        fill(0, min(255, this.faalAnimatie*20)); 
        textSize(70 + this.faalAnimatie / 10); // tekst groter maken
...
function preload() {
    ...
    pngSnelweg = loadSound('assets/snelweg.mp3');
    pngBloed = loadImage('assets/bloed.png');
